...
**Current state:** Phase 7.5 cross-platform hardening in progress. Windows is release-built; Ubuntu `.deb` packaging is verified; AppImage and macOS packaging remain pending.
...
- **Linux `.deb` packaging:** Ubuntu 22.04 builds the frontend, passes sidecar tests, builds the sidecar binary, and bundles the Debian package line; the next release build will emit `0.7.5`.
