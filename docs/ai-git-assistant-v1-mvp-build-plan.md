...
**Status:** Superseded — see `AI_Git_Assistant_v1_MVP_Build_Plan.md` for the current version with phase completion markers.
...
| 5 LLM providers wired before launch | Ollama + one cloud provider (Groq) | Proves the provider abstraction works with a free local option and a free cloud option. Gemini, Claude, OpenAI plug into the same interface later, same code shape, just more adapters. |
...
| Tauri Stronghold for secret storage | OS keychain via a lightweight Tauri plugin for v1, Stronghold considered later | Stronghold is a full secure vault, heavier than needed for storing a handful of API keys. Confirm its current docs and maintenance status yourself before adopting it; an OS-keychain-backed store gets the same core property (encrypted, not in SQLite, not in plaintext config) with less integration surface. |
...
**Phase 1 — Foundation and read-only**
...
**Phase 2 — Plans and writes**
- Plan model, risk engine, preview card, approve/cancel flow, stale-plan recheck
- `git_stage_paths`, `git_stash_push`, `git_switch_branch`, `git_create_branch`, `git_commit`, `git_pull_ff_only`, `git_push`
- Multi-repository switching
- Basic tests against disposable test repos
- **Milestone: the full daily workflow works end to end with approval gates**

**Phase 3 — Providers and polish**
- Ollama provider, then Groq provider, both through the shared interface
- Settings UI, per-repository external-sharing toggle
- Packaging spike, Windows installer, clean startup/shutdown without Python installed
- Error-state polish, README and portfolio write-up
- **Milestone: shippable v1, demoable, installable on a clean Windows machine**
